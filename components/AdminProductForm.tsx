...
export default function AdminProductForm({
  ...
  return (
    <form onSubmit={saveProduct} className="space-y-6 pb-24">
      ...
      <section className="admin-panel">
        ...
        {variants.length === 0 ? <label className="admin-label mt-5">Product inventory<input value={inventory} onChange={(event) => setInventory(event.target.value)} className="admin-input" type="number" min="0" step="1" /></label> : <div className="mt-5 space-y-3">{variants.map((variant, index) => <div key={variant.id} className="border border-black/10 p-4"><div className="grid gap-3 sm:grid-cols-[1fr_1fr_100px_auto]"><input value={variant.name} onChange={(event) => updateVariant(variant.id, { name: event.target.value })} className="admin-input" aria-label="Variant name" /><input value={variant.sku || ""} onChange={(event) => updateVariant(variant.id, { sku: event.target.value })} className="admin-input" placeholder="SKU" aria-label="Variant SKU" /><input value={variant.inventory_quantity} onChange={(event) => updateVariant(variant.id, { inventory_quantity: Math.max(0, Number(event.target.value)) })} className="admin-input" type="number" min="0" aria-label="Variant quantity" /><div className="flex gap-1"><button type="button" onClick={() => moveVariant(variant.id, -1)} disabled={index === 0} className="min-h-12 min-w-12 border border-black/15 disabled:opacity-30" aria-label="Move variant up">↑</button><button type="button" onClick={() => moveVariant(variant.id, 1)} disabled={index === variants.length - 1} className="min-h-12 min-w-12 border border-black/15 disabled:opacity-30" aria-label="Move variant down">↓</button><button type="button" onClick={() => setVariants((current) => current.filter((item) => item.id !== variant.id))} className="min-h-12 min-w-12 border border-black/15 text-[var(--burgundy)]" aria-label={`Remove ${variant.name}`}>×</button></div></div></div>)}</div>}
